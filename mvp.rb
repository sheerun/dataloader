...
class BatchPromise < Promise
  ...
  def queue(key)
    if @dispatched
      raise StandardError.new("Cannot queue elements after batch is dispatched. Queued key: #{key}")
    end
    ...
    @dispatch.then do |values|
      unless values.key?(key)
        raise StandardError.new(
          "Promise didn't resolve a key: #{key}\nResolved keys: #{values.keys.join(' ')}"
        )
      end
  ...
  def handle_result(keys, values)
    unless values.is_a?(Array) || values.is_a?(Hash)
      raise TypeError.new(
        'DataLoader must be constructed with a block which accepts ' +
    ...
    if keys.size != values.size
      raise TypeError.new(
        'DataLoader must be instantiated with function that returns Array or Hash ' +
...
class DataLoader
  ...
  def load(key)
    if key.nil?
      raise TypeError.new(
        "The loader.load() must be called with a key, but got: #{key}"
      )
    end

    cache_key_fn = @options.fetch(:key, lambda { |key| key })

    if cache_key_fn.respond_to?(:call)
      cache_key = cache_key_fn.call(key)
    else
      cache_key = key[cache_key_fn]
    end

    @promises.compute_if_absent(cache_key) {
      batch = batch_promise
      batch.queue(key)
    }
  end
  ...
  def load_many(keys)
    unless keys.kind_of?(Array)
      raise TypeError.new(
        "The loader.load_many() must be called with a Array<key>, but got: #{key}"
      )
    end
